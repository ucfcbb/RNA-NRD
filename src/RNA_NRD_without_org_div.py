...
def generate_output_without_org(output_without_org):
    ...
    print("\n ------- Generate RNA-NRD_without_Organism_division dataset along with representative for each cluster (without organism division) ------- \n")
    ...
    #### Read current organism information
    ...
    #### Read all organism information
    f_organism_all = open("Organism_list/All_Organism_list", "r")
    All_organism_list = []

    while(True):
        line = f_organism_all.readline()
...
##                organism_list.append(chain_info[line_list[li]][8])
            ...
            org = chain_info[line_list[li]][8]
            for o in All_organism_list:
                cur_og = o
                cur_org_len = len(cur_og)

                if cur_og == org[:cur_org_len]:
                    FLAG = 1
                    if cur_og not in organism_list:
                        organism_list.append(cur_og)
                        
                    break
            if FLAG == 0 and org != '-':
                organism_list.append(org)
                print("no match: ", line_list[li], group_id, org)
