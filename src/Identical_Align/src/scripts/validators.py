...
def get_node_list_from_rmsd_data(rmsd_data_dict):
    ...
    for c_id in rmsd_data_dict:
        ...
        key_list = rmsd_data_list_dict.keys()

        if len(key_list) > 0:
            (i, r1) = key_list[0]
            node1 = strToNode(r1)
            _, fit_ret = rmsd_data_list_dict[(i, r1)]
            node2_list = map(lambda x: strToNode(x[1]), fit_ret)
...
def is_valid_pickle(pickle_fname, clusters):
    ...
    if os.path.basename(pickle_fname).startswith('alignment'):
        ...
        for c_id in clusters:
            loop_nodes = map(lambda x: strToNode(x), clusters[c_id])
            node1 = loop_nodes[0]
            set_a = set(loop_nodes)
            set_b = set(cluster_alignment_data[c_id][node1].keys() + [node1])
    ...
    if draw_figures == True:
        if sys.version_info > (3, 0):
            if len(pymol_py3_dir) == 0:
                logger.error('Please configure pymol and set pymol directory to generate images.')
                sys.exit()
        else:
            try:
                import pymol
            except Exception as e:
                logger.error('Please install pymol to generate images.')
                sys.exit()
