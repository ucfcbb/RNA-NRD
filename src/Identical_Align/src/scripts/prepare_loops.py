...
def download_single_pdbx_or_fasta_file(fname, file_ext):
    ...
    urlretrieve('https://files.rcsb.org/download/6ypu.cif', 'input_files/6ypu.cif')
    ...
    while status == False and number_of_tries <= download_attempts:
        ...
        if file_ext == 'cif':
            ...
            # print(fname)
            # print(file_ext)
            #urlretrieve(pdbx_url + fname, pdbx_dir + fname)
            #urlretrieve('https://files.rcsb.org/download/6ypu.cif', 'input_files/6ypu.cif')
            wait_for_certain_time_according_to_wait_factor(wait_time)
            fp = open(os.path.join(pdbx_dir, fname))
            lines = fp.readlines()
            fp.close()
            if lines[0].startswith('data_' + fname.upper().strip().split('.')[0]) and has_complete_ATOM_portion(lines) and lines[-1].strip() == '#':
                status = True
            ...
            lines = fp.readlines()
            fp.close()
            if lines[0].startswith('>' + fname.upper().strip().split('.')[0]):
                status = True
        else:
...
def get_fasta_sequence(pdb_id, chains):
    # print(pdb_id)
    # print(chains)
    ...
    if not os.path.isfile(fasta_fn):
        logger.error(pdb_id + '.fasta' + ' file is missing.')
        return

    #residue_dict, missing_residue_dict, modified_residue_dict = load_pdb_data_from_file(pdb_fn)

    # for res in chains:
    #     print(res,residue_dict[res])
    # for res in missing_residue_dict:
    #     print(res, missing_residue_dict[res])
    # for res in modified_residue_dict:
    #     print(res, modified_residue_dict[res])
