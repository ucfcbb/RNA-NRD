...
def extract_atom_coordinate(loop_coord, pdb_pm, pdb_id):
    ...
    for chain_id, index, icode in pdb_pm:
        if chain_id == '':
            continue
        if loop_coord_backbone[(pdb_id, chain_id, index)] != None:
            aligned_segment_coord.append(loop_coord_backbone[(pdb_id, chain_id, index)])
        if loop_coord_sugar[(pdb_id, chain_id, index)] != None:
            aligned_segment_coord.append(loop_coord_sugar[(pdb_id, chain_id, index)])
